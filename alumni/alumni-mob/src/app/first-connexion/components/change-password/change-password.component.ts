...
export class ChangePasswordComponent implements OnInit {
  ...
  onSubmit() {
    if (this.form.value.password === this.form.value.confirmPassword) {
      ...
      if (email === null) {
        this.createToast(
          'Une erreur est survenue veuillez réessayer tout le processus',
          ''
        );
      }
      if (verificationCode) {
        this._mailerService
          .changePassword(email, this.form.value.password, verificationCode)
          .pipe(take(1))
          .subscribe({
            next: async (response: HttpResponse<any>) => {
              if (response.status === 204) {
                ...
                this.createToast(
                  `Une erreur est survenue lors de l'initilaisation de votre mot de passe veuillez réessayer`,
                  'Réessayer'
                );
              }
            },
